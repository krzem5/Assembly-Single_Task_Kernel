...
static _Bool _load_location(s64 fd,char* location){
	fd=sys_fd_open(fd,"location",SYS_FD_FLAG_READ);
	if (fd<=0){
		return 0;
	}
	s64 size=sys_fd_read(fd,location,47,0);
	sys_fd_close(fd);
	if (size>0){
		location[size]=0;
	}
...
int main(int argc,const char** argv){
	...
	for (s64 iter=sys_fd_iter_start(types_fd);iter>=0;iter=sys_fd_iter_next(iter)){
		...
		s64 fd=sys_fd_open(types_fd,name,0);
		if (fd<=0){
			continue;
		}
		_load_location(fd,(types+i)->location);
		sys_fd_close(fd);
		printf("%u: %s\n",i,(types+i)->location);
	}
	...
	for (s64 iter=sys_fd_iter_start(data_fd);iter>=0;iter=sys_fd_iter_next(iter)){
		...
		s64 fd=sys_fd_open(data_fd,name,0);
		if (fd<=0){
			continue;
		}
		char location[48];
		if (!_load_location(fd,location)){
			sys_fd_close(fd);
			continue;
		}
		printf("%s\n",location);
