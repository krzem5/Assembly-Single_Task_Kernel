...
static vfs_node_t* _sysfs_handle_root;
...
static void _listener(void* object,u32 type){
	...
	if (type==NOTIFICATION_TYPE_HANDLE_CREATE){
		...
		vfs_node_t* node=dynamicfs_create_node(_sysfs_handle_root,buffer,VFS_NODE_TYPE_DIRECTORY,NULL,NULL,NULL);
...
void sysfs_handle_init(void){
	...
	_sysfs_handle_root=dynamicfs_create_node(sysfs->root,"handle",VFS_NODE_TYPE_DIRECTORY,NULL,NULL,NULL);
