...
static void _listener(void* object,u32 type){
	...
	if (type==NOTIFICATION_TYPE_HANDLE_CREATE){
		...
		vfs_node_t* node=dynamicfs_create_node(_devfs_partition_root,buffer,VFS_NODE_TYPE_DIRECTORY,NULL);
		dynamicfs_create_node(node,"name",VFS_NODE_TYPE_FILE,smm_duplicate(partition->name));
...
void devfs_partition_init(void){
	...
	_devfs_partition_root=dynamicfs_create_node(devfs->root,"partition",VFS_NODE_TYPE_DIRECTORY,NULL);
