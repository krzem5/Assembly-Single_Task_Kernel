...
static void _listener(void* object,u32 type){
	...
	if (type==NOTIFICATION_TYPE_HANDLE_CREATE){
		...
		vfs_node_t* node=dynamicfs_create_node(_devfs_drive_root,buffer,VFS_NODE_TYPE_DIRECTORY,NULL);
		dynamicfs_create_node(node,"serial_number",VFS_NODE_TYPE_FILE,smm_duplicate(drive->serial_number));
		dynamicfs_create_node(node,"model_number",VFS_NODE_TYPE_FILE,smm_duplicate(drive->model_number));
		...
		dynamicfs_create_node(node,"partition",VFS_NODE_TYPE_DIRECTORY,NULL);
		return;
	}
...
void devfs_drive_init(void){
	...
	_devfs_drive_root=dynamicfs_create_node(devfs->root,"drive",VFS_NODE_TYPE_DIRECTORY,NULL);
