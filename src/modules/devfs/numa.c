...
void devfs_numa_init(void){
	...
	for (u32 i=0;i<numa_node_count;i++){
		...
		for (const numa_cpu_t* numa_cpu=numa_node->cpus;numa_cpu;numa_cpu=numa_cpu->next){
			format_string(buffer,16,"cpu%u",j);
			vfs_node_t* cpu=dynamicfs_create_node(device_root,buffer,VFS_NODE_TYPE_DIRECTORY,NULL,NULL,NULL);
			dynamicfs_create_data_node(cpu,"id","%u",numa_cpu->apic_id);
			dynamicfs_create_data_node(cpu,"hot_pluggable","0");
			j++;
		}
		j=0;
		for (const numa_memory_range_t* numa_memory=numa_node->memory_ranges;numa_memory;numa_memory=numa_memory->next){
			format_string(buffer,16,"mem%u",j);
			vfs_node_t* memory=dynamicfs_create_node(device_root,buffer,VFS_NODE_TYPE_DIRECTORY,NULL,NULL,NULL);
			dynamicfs_create_data_node(memory,"address","%lu",numa_memory->base_address);
			dynamicfs_create_data_node(memory,"length","%lu",numa_memory->length);
			dynamicfs_create_data_node(memory,"hot_pluggable","%u",numa_memory->hot_pluggable);
			j++;
		}
		...
		for (j=0;j<numa_node_count;j++){
			format_string(buffer,16,"%u",j);
			dynamicfs_create_data_node(locality_root,buffer,"%u",numa_node_locality_matrix[NUMA_LOCALITY_INDEX(i,j)]);
		}
	}
}
