...
def _compile_uefi():
	...
	for file in _get_files([UEFI_FILE_DIRECTORY,COMMON_FILE_DIRECTORY+"/compressor",COMMON_FILE_DIRECTORY+"/kfs2"]):
		...
		command=None
		if (file.endswith(".c")):
			command=["gcc-12",f"-I{COMMON_FILE_DIRECTORY}/include",f"-I{COMMON_FILE_DIRECTORY}/compressor/include",f"-I{COMMON_FILE_DIRECTORY}/kernel_data/include",f"-I{COMMON_FILE_DIRECTORY}/kfs2/include","-I/usr/include/efi","-I/usr/include/efi/x86_64","-fno-stack-protector","-ffreestanding","-O3","-fpic","-fshort-wchar","-mno-red-zone","-maccumulate-outgoing-args","-fdiagnostics-color=always","-DBUILD_UEFI=1","-DGNU_EFI_USE_MS_ABI","-Dx86_64","-m64","-Wall","-Werror","-Wno-trigraphs","-Wno-address-of-packed-member","-DNULL=((void*)0)","-o",object_file,"-c",file,f"-I{UEFI_FILE_DIRECTORY}/include"]
		else:
...
def _compile_module(module,dependencies,changed_files,pool):
	...
	for file in _get_files([MODULE_FILE_DIRECTORY+"/"+module]+[COMMON_FILE_DIRECTORY+"/"+tag.name for tag in dependencies.iter() if tag.data==b"common"]):
		...
		command=None
		if (file.endswith(".c")):
			command=["gcc-12",f"-I{COMMON_FILE_DIRECTORY}/include",f"-I{COMMON_FILE_DIRECTORY}/kernel_data/include","-mcmodel=kernel","-mno-red-zone","-mno-mmx","-mno-sse","-mno-sse2","-mbmi","-mbmi2","-fno-common","-fno-builtin","-nostdlib","-fno-omit-frame-pointer","-fno-asynchronous-unwind-tables","-ffreestanding",f"-fvisibility={KERNEL_SYMBOL_VISIBILITY}","-fplt","-fno-pie","-fno-pic","-m64","-Wall","-Werror","-Wno-trigraphs","-Wno-address-of-packed-member","-c","-o",object_file,"-fdiagnostics-color=always",file,"-DNULL=((void*)0)","-DBUILD_MODULE=1"]+included_directories+MODULE_EXTRA_COMPILER_OPTIONS
		else:
