...
if ("--run" in sys.argv):
	if (not NO_FILE_SERVER):
		subprocess.Popen((["/usr/libexec/virtiofsd",f"--socket-group={os.getlogin()}"] if not os.getenv("GITHUB_ACTIONS","") else ["sudo","build/external/virtiofsd/target/release/virtiofsd"])+["--socket-path=build/virtiofsd.sock","--shared-dir","build/share","--inode-file-handles=mandatory"])
