...
def _compile_library(library,flags,dependencies):
	...
	for root,_,files in os.walk(LIBRARY_FILE_DIRECTORY+"/"+library):
		for file_name in files:
			...
			command=None
			if (suffix==".c"):
				command=["gcc-12","-fno-common","-fno-builtin","-nostdlib","-fvisibility=hidden","-ffreestanding","-shared","-fpic","-m64","-Wall","-Werror","-c","-o",object_file,"-c",file,"-DNULL=((void*)0)"]+included_directories+LIBRARY_EXTRA_COMPILER_OPTIONS
			else:
	...
	if (error):
		sys.exit(1)
	if ("nodynamic" not in flags and subprocess.run(["ld","-znoexecstack","-melf_x86_64","-L","build/lib","--exclude-libs","ALL","-shared","-o",f"build/lib/lib{library}.so"]+object_files+[(f"-l{dep.split('@')[0]}" if "@dynamic" in dep else f"build/lib/lib{dep.split('@')[0]}.a") for dep in dependencies]+LIBRARY_EXTRA_LINKER_OPTIONS).returncode!=0):
		sys.exit(1)
	...
	for root,_,files in os.walk(USER_FILE_DIRECTORY+"/"+program):
		for file_name in files:
			...
			if (subprocess.run(command+["-MD","-MT",object_file,"-MF",object_file+".deps"]).returncode!=0):
				...
				error=True
	_save_file_hash_list(file_hash_list,hash_file_path)
	if (error or subprocess.run(["ld","-znoexecstack","-melf_x86_64","-rpath","build/lib","-I/lib/ld.so","-T","src/user/linker.ld","-o",f"build/user/{name}"]+[(f"-l{dep[0]}" if len(dep)==1 or dep[1]!="static" else f"build/lib/lib{dep[0]}.a") for dep in dependencies]+object_files+USER_EXTRA_LINKER_OPTIONS).returncode!=0):
		sys.exit(1)
...
for root,_,files in os.walk(KERNEL_FILE_DIRECTORY):
	for file_name in files:
		...
		if (suffix==".c"):
			command=["gcc-12","-mcmodel=kernel","-mno-red-zone","-mno-mmx","-mno-sse","-mno-sse2","-mbmi","-mbmi2","-fno-lto","-fplt","-fno-pie","-fno-pic","-fno-common","-fno-builtin","-fno-stack-protector","-fno-asynchronous-unwind-tables","-nostdinc","-nostdlib","-ffreestanding","-fvisibility=hidden","-m64","-Wall","-Werror","-Wno-address-of-packed-member","-c","-ftree-loop-distribute-patterns","-O3","-g0","-fno-omit-frame-pointer","-DNULL=((void*)0)","-o",object_file,"-c",file,f"-I{KERNEL_FILE_DIRECTORY}/include"]+KERNEL_EXTRA_COMPILER_OPTIONS
		else:
			command=["nasm","-f","elf64","-O3","-Wall","-Werror","-o",object_file,file]
		print(file)
		if (subprocess.run(command+["-MD","-MT",object_file,"-MF",object_file+".deps"]).returncode!=0):
