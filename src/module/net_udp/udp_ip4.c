...
static omm_allocator_t* KERNEL_INIT_WRITE _net_udp_ip4_address_allocator=NULL;



static bool _socket_bind_callback(socket_vfs_node_t* socket_node,const void* address,u32 address_length){
	const net_udp_ip4_address_t* udp_address=(const net_udp_ip4_address_t*)address;
	if (address_length!=sizeof(net_udp_ip4_address_t)||!socket_port_reserve(socket_node,udp_address->port)){
		return 0;
	}
	net_udp_ip4_address_t* local_ctx=omm_alloc(_net_udp_ip4_address_allocator);
...
static bool _socket_connect_callback(socket_vfs_node_t* socket_node,const void* address,u32 address_length){
	...
	net_udp_ip4_address_t* remote_ctx=omm_alloc(_net_udp_ip4_address_allocator);
...
static KERNEL_AWAITS u64 _socket_write_callback(socket_vfs_node_t* socket_node,const void* buffer,u64 length){
	...
	const net_udp_ip4_address_t* udp_local_address=(const net_udp_ip4_address_t*)(socket_node->local_ctx);
	const net_udp_ip4_address_t* udp_remote_address=(const net_udp_ip4_address_t*)(socket_node->remote_ctx);
...
static KERNEL_AWAITS bool _socket_write_packet_callback(socket_vfs_node_t* socket_node,const void* buffer,u32 length){
	if (length<sizeof(net_udp_ip4_socket_packet_t)){
		return 0;
	}
	const net_udp_ip4_socket_packet_t* packet=(const net_udp_ip4_socket_packet_t*)buffer;
	if (packet->length+sizeof(net_udp_ip4_socket_packet_t)>length){
		return 0;
	}
	net_ip4_packet_t* ip_packet=net_ip4_create_packet(packet->length+sizeof(net_udp_packet_t),packet->src_address.address,packet->dst_address.address,PROTOCOL_TYPE);
	if (!ip_packet){
		return 0;
	}
	net_udp_packet_t* udp_packet=(net_udp_packet_t*)(ip_packet->packet->data);
...
static KERNEL_AWAITS void _rx_callback(net_ip4_packet_t* packet){
	...
	net_udp_ip4_socket_packet_t* socket_packet=amm_alloc(sizeof(net_udp_ip4_socket_packet_t)+packet->length-sizeof(net_udp_packet_t));
	...
	if (!socket_alloc_packet(&(socket->node),socket_packet,sizeof(net_udp_ip4_socket_packet_t)+packet->length-sizeof(net_udp_packet_t))){
		amm_dealloc(packet);
...
MODULE_INIT(){
	_net_udp_ip4_address_allocator=omm_init("net.udp.ip4.address",sizeof(net_udp_ip4_address_t),8,4);
	rwlock_init(&(_net_udp_ip4_address_allocator->lock));
}
