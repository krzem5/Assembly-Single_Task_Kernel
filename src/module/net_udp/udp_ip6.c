...
#include <net/ip6.h>
#include <net/udp.h>
#define KERNEL_LOG_NAME "net_udp_ip6"
...
static omm_allocator_t* KERNEL_INIT_WRITE _net_udp_ip6_address_allocator=NULL;



static bool _socket_bind_callback(socket_vfs_node_t* socket_node,const void* address,u32 address_length){
	const net_udp_ip6_address_t* udp_address=(const net_udp_ip6_address_t*)address;
	if (address_length!=sizeof(net_udp_ip6_address_t)||!socket_port_reserve(socket_node,udp_address->port)){
		return 0;
	}
	net_udp_ip6_address_t* local_ctx=omm_alloc(_net_udp_ip6_address_allocator);
...
static void _socket_debind_callback(socket_vfs_node_t* socket_node){
	panic("UDP:IPv6:_socket_debind_callback");
}
...
static bool _socket_connect_callback(socket_vfs_node_t* socket_node,const void* address,u32 address_length){
	...
	net_udp_ip6_address_t* remote_ctx=omm_alloc(_net_udp_ip6_address_allocator);
...
static void _socket_deconnect_callback(socket_vfs_node_t* socket_node){
	panic("UDP:IPv6:_socket_deconnect_callback");
}
...
static KERNEL_AWAITS u64 _socket_read_callback(socket_vfs_node_t* socket_node,void* buffer,u64 length,u32 flags){
	...
	net_udp_ip6_socket_packet_t* packet=socket_packet->data;
...
static KERNEL_AWAITS u64 _socket_write_callback(socket_vfs_node_t* socket_node,const void* buffer,u64 length){
	...
	// const net_udp_ip6_address_t* udp_local_address=(const net_udp_ip6_address_t*)(socket_node->local_ctx);
	// const net_udp_ip6_address_t* udp_remote_address=(const net_udp_ip6_address_t*)(socket_node->remote_ctx);
	// net_ip6_packet_t* ip_packet=net_ip6_create_packet(length+sizeof(net_udp_packet_t),udp_local_address->address,udp_remote_address->address,PROTOCOL_TYPE);
	// if (!ip_packet){
	// 	return 0;
	// }
...
static bool _socket_write_packet_callback(socket_vfs_node_t* socket_node,const void* buffer,u32 length){
	if (length<sizeof(net_udp_ip6_socket_packet_t)){
		return 0;
	}
	const net_udp_ip6_socket_packet_t* packet=(const net_udp_ip6_socket_packet_t*)buffer;
	if (packet->length+sizeof(net_udp_ip6_socket_packet_t)>length){
		return 0;
	}
	// net_ip6_packet_t* ip_packet=net_ip6_create_packet(packet->length+sizeof(net_udp_packet_t),packet->src_address.address,packet->dst_address.address,PROTOCOL_TYPE);
	// if (!ip_packet){
	// 	return 0;
	// }
	// net_udp_packet_t* udp_packet=(net_udp_packet_t*)(ip_packet->packet->data);
...
static void _rx_callback(net_ip6_packet_t* packet){
	...
	ERROR("UDP:IPv6 packet");
	if (udp_packet->checksum){
		panic("UDP:IPv6 checksum");
		// net_udp_ipv4_pseudo_header_t pseudo_header={
	...
	if (!socket||socket->descriptor!=&_net_udp_socket_dtp_descriptor){
		ERROR("No UDP:IPv6 socket on port %u",port);
		if (socket){
			vfs_node_unref(&(socket->node));
		}
		return;
	}
	// net_udp_ip6_socket_packet_t* socket_packet=amm_alloc(sizeof(net_udp_ip6_socket_packet_t)+packet->length-sizeof(net_udp_packet_t));
	// socket_packet->src_address.address=__builtin_bswap32(packet->packet->src_address);
	// socket_packet->src_address.port=__builtin_bswap16(udp_packet->src_port);
	// socket_packet->dst_address.address=__builtin_bswap32(packet->packet->dst_address);
	...
	// if (!socket_alloc_packet(&(socket->node),socket_packet,sizeof(net_udp_ip6_socket_packet_t)+packet->length-sizeof(net_udp_packet_t))){
	// 	amm_dealloc(packet);
...
static const net_ip6_protocol_descriptor_t _net_udp_ip6_protocol_descriptor={
	"UDP (IPv6)",
	PROTOCOL_TYPE,
	_rx_callback
};
...
MODULE_INIT(){
	_net_udp_ip6_address_allocator=omm_init("net.udp.ip6.address",sizeof(net_udp_ip6_address_t),8,4);
	rwlock_init(&(_net_udp_ip6_address_allocator->lock));
}



MODULE_POSTINIT(){
	LOG("Registering UDP over IPv6 protocol...");
	socket_register_dtp_descriptor(&_net_udp_socket_dtp_descriptor);
	net_ip6_register_protocol_descriptor(&_net_udp_ip6_protocol_descriptor);
}
