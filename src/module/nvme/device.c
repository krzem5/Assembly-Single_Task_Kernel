...
static u64 _nvme_read_write(drive_t* drive,u64 offset,u64 buffer,u64 count){
...
MODULE_INIT(){
	_nvme_driver_pmm_counter=pmm_alloc_counter("nvme.driver");
	_nvme_device_allocator=omm_init("nvme.device",sizeof(nvme_device_t),8,1);
...
MODULE_POSTINIT(){
	HANDLE_FOREACH(pci_device_handle_type){
		_nvme_init_device(KERNEL_CONTAINEROF(handle,pci_device_t,handle));
	}
}
