...
static const u32 _gpt_crc_table[256]={
...
static u32 _calculate_crc(const void* data,u32 size){
	const u8* ptr=data;
	u32 out=0xffffffff;
	for (u32 i=0;i<size;i++){
		out=_gpt_crc_table[(out&0xff)^ptr[i]]^(out>>8);
	}
	return ~out;
}
...
static bool _gpt_load_partitions(drive_t* drive){
	...
	u32 crc=header->crc;
	header->crc=0;
	if (_calculate_crc(header,header->header_size)!=crc){
		ERROR("Incorrect crc");
		goto _cleanup;
	}
...
static bool _gpt_format_drive(drive_t* drive){
	u64 partition_table_size=(FORMAT_PARITION_TABLE_ENTRY_COUNT*sizeof(gpt_partition_entry_t)+drive->block_size-1)&(-drive->block_size);
	u64 required_space=3+2*(partition_table_size>>drive->block_size_shift);
	if (drive->block_count<=required_space){
		return 0;
	}
	void* entry_buffer=(void*)(pmm_alloc(pmm_align_up_address(partition_table_size)>>PAGE_SIZE_SHIFT,_gpt_driver_pmm_counter,0)+VMM_HIGHER_HALF_ADDRESS_OFFSET);
	bool out=0;
	if (drive_write(drive,2,entry_buffer,partition_table_size>>drive->block_size_shift)!=(partition_table_size>>drive->block_size_shift)||drive_write(drive,drive->block_count-(partition_table_size>>drive->block_size_shift)-1,entry_buffer,partition_table_size>>drive->block_size_shift)!=(partition_table_size>>drive->block_size_shift)){
		goto _cleanup;
	}
	u32 crc=_calculate_crc(entry_buffer,partition_table_size);
	gpt_table_header_t* header=(void*)entry_buffer;
	header->signature=GPT_TABLE_HEADER_SIGNATURE;
	header->revision=0x00010000;
	header->header_size=sizeof(gpt_table_header_t);
	header->crc=0;
	header->_zero=0;
	...
	header->entry_crc=crc;
	header->crc=_calculate_crc(header,sizeof(gpt_table_header_t));
	if (drive_write(drive,1,header,1)!=1){
		goto _cleanup;
	}
	header->entry_lba=drive->block_count-2;
	header->crc=_calculate_crc(header,sizeof(gpt_table_header_t));
	if (drive_write(drive,1,header,1)!=1){
		goto _cleanup;
	}
	out=1;
_cleanup:
	pmm_dealloc(((u64)entry_buffer)-VMM_HIGHER_HALF_ADDRESS_OFFSET,pmm_align_up_address(partition_table_size)>>PAGE_SIZE_SHIFT,_gpt_driver_pmm_counter);
	return out;
}
