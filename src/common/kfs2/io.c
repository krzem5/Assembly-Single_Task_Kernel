...
void kfs2_io_inode_write(kfs2_filesystem_t* fs,const kfs2_node_t* node){
	void* buffer=fs->config.alloc_callback(1);
	if (fs->config.read_callback(fs->config.ctx,fs->config.start_lba+((fs->root_block.first_inode_block+KFS2_INODE_GET_BLOCK_INDEX(node->inode))<<fs->block_size_shift),buffer,1<<fs->block_size_shift)!=(1<<fs->block_size_shift)){
		panic("kfs2_io_inode_write: I/O error");
	}
	kfs2_node_t* tmp_node=buffer+KFS2_INODE_GET_NODE_INDEX(node->inode)*sizeof(kfs2_node_t);
	*tmp_node=*node;
	kfs2_insert_crc(tmp_node,sizeof(kfs2_node_t));
	if (fs->config.write_callback(fs->config.ctx,fs->config.start_lba+((fs->root_block.first_inode_block+KFS2_INODE_GET_BLOCK_INDEX(node->inode))<<fs->block_size_shift),buffer,1<<fs->block_size_shift)!=(1<<fs->block_size_shift)){
		panic("kfs2_io_inode_write: I/O error");
	}
	fs->config.dealloc_callback(buffer,1);
}
