...
static int _node_to_fd(fs_node_t* node,u8 flags){
	...
	data->offset=((flags&FD_FLAG_APPEND)?fs_node_get_size(node):0);
...
int fd_open(fd_t root,const char* path,u32 length,u8 flags){
	...
	if (root){
		...
		root_node=fs_node_get_by_id(_get_fd_data(root)->node_id);
		if (!root_node){
			lock_release(&_fd_lock);
	...
	buffer[length]=0;
	fs_node_t* node=fs_node_get_by_path(root_node,buffer,((flags&FD_FLAG_CREATE)?((flags&FD_FLAG_DIRECTORY)?FS_NODE_TYPE_DIRECTORY:FS_NODE_TYPE_FILE):0));
	lock_release(&_fd_lock);
	if (!node){
		return FD_ERROR_NOT_FOUND;
	}
...
int fd_delete(fd_t fd){
	...
	fs_node_t* node=fs_node_get_by_id(data->node_id);
	if (!node){
		lock_release(&_fd_lock);
		return FD_ERROR_NOT_FOUND;
	}
...
s64 fd_read(fd_t fd,void* buffer,u64 count){
	...
	count=fs_node_read(node,data->offset,buffer,count);
	data->offset+=count;
	lock_release(&_fd_lock);
	return count;
}
...
s64 fd_write(fd_t fd,const void* buffer,u64 count){
	...
	count=fs_node_write(node,data->offset,buffer,count);
	data->offset+=count;
	lock_release(&_fd_lock);
	return count;
}
...
s64 fd_seek(fd_t fd,u64 offset,u8 flags){
	...
	switch (flags){
		...
		case FD_SEEK_END:
			...
			data->offset=fs_node_get_size(node);
			break;
		default:
			lock_release(&_fd_lock);
			return FD_ERROR_INVALID_FLAGS;
...
int fd_stat(fd_t fd,fd_stat_t* out){
	...
	out->size=fs_node_get_size(node);
	lock_release(&_fd_lock);
	return 0;
}
...
int fd_get_relative(fd_t fd,u8 relative,u8 flags){
	...
	fs_node_t* other=fs_node_get_relative(node,relative);
	lock_release(&_fd_lock);
	if (!other){
...
int fd_move(fd_t fd,fd_t dst_fd){
	...
	fs_node_t* dst_node=fs_node_get_by_id(dst_data->node_id);
	if (!dst_node){
		lock_release(&_fd_lock);
		return FD_ERROR_NOT_FOUND;
	}
	...
	if (node->flags&FS_NODE_TYPE_DIRECTORY){
		if (fs_node_get_relative(dst_node,FS_NODE_RELATIVE_FIRST_CHILD)){
			lock_release(&_fd_lock);
			return FD_ERROR_NOT_EMPTY;
		}
	}
	else{
		if (fs_node_get_size(dst_node)){
			lock_release(&_fd_lock);
			return FD_ERROR_NOT_EMPTY;
		}
	}
	_Bool out=fs_node_move(node,dst_node);
	lock_release(&_fd_lock);
	return (out?0:FD_ERROR_NOT_EMPTY);
}
