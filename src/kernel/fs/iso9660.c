...
static vfs_node_t* _iso9660_create(partition_t* fs,_Bool is_directory,const char* name,u8 name_length){
	return NULL;
}



static _Bool _iso9660_delete(partition_t* fs,vfs_node_t* node){
	return 0;
}
...
_skip_directory_entry:
	...
	if (relative==VFS_RELATIVE_NEXT_SIBLING){
...
static _Bool _iso9660_set_relative(partition_t* fs,vfs_node_t* node,u8 relative,vfs_node_t* other){
	return 0;
}



static _Bool _iso9660_move_file(partition_t* fs,vfs_node_t* src_node,vfs_node_t* dst_node){
	return 0;
}
...
static u64 _iso9660_write(partition_t* fs,vfs_node_t* node,u64 offset,const u8* buffer,u64 count){
	return 0;
}



static u64 _iso9660_get_size(partition_t* fs,vfs_node_t* node){
	const iso9660_fs_node_t* iso9660_node=(const iso9660_fs_node_t*)node;
...
static _Bool _iso9660_set_size(partition_t* fs,vfs_node_t* node,u64 size){
	return 0;
}
