...
void syscall_scheduler_get_timers(syscall_registers_t* regs){
	if (sandbox_get(THREAD_DATA->sandbox,SANDBOX_FLAG_DISABLE_SCHEDULER_API)||regs->rdx!=sizeof(scheduler_timers_t)||!syscall_sanatize_user_memory(regs->rsi,regs->rdx)||!scheduler_get_timers(regs->rdi,(void*)(regs->rsi))){
		regs->rax=0;
		return;
	}
	regs->rax=1;
