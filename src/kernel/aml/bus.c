...
static void _enumerate_bus(aml_node_t* bus){
	for (aml_node_t* device=bus->child;device;device=device->next){
		...
		u64 status=0xf;
		aml_node_t* status_node=aml_runtime_evaluate(aml_get_node(device,"_STA"));
		if (status_node){
		...
		if (status&1){
			...
			aml_node_t* hid=aml_runtime_evaluate(aml_get_node(device,"_HID"));
			if (hid){
				aml_print_node(hid);
			}
			aml_node_t* adr=aml_runtime_evaluate(aml_get_node(device,"_ADR"));
			if (adr){
				aml_print_node(adr);
			}
			aml_node_t* crs=aml_runtime_evaluate(aml_get_node(device,"_CRS"));
...
void aml_enumerate_bus(void){
	...
	_enumerate_bus(aml_get_node(aml_root_node,"\\_SB_"));
	for (;;);
}
