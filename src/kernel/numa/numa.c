...
void numa_init(u32 proximity_domain_count){
	...
	numa_nodes=kmm_allocate(numa_node_count*sizeof(numa_node_t));
	numa_node_locality_matrix=kmm_allocate(numa_node_count*numa_node_count*sizeof(u8));
...
void numa_add_cpu(u32 node_index,u8 apic_id,u32 sapic_eid){
	...
	numa_cpu_t* cpu=kmm_allocate(sizeof(numa_cpu_t));
...
void numa_add_memory_range(u32 node_index,u64 base_address,u64 length,_Bool hot_pluggable){
	...
	numa_memory_range_t* memory_range=kmm_allocate(sizeof(numa_memory_range_t));
