...
#define PMM_ALLOCATOR_BUCKET_COUNT_MASK_SHIFT (31-__builtin_clz(PMM_ALLOCATOR_BUCKET_COUNT))
#define PMM_ALLOCATOR_BUCKET_COUNT_MASK (PMM_ALLOCATOR_BUCKET_COUNT-1)
...
#define PMM_ALLOCATOR_BLOCK_DESCRIPTOR_FLAG_CLEAR (1<<(PMM_ALLOCATOR_BUCKET_COUNT_MASK_SHIFT<<1))
...
typedef struct _PMM_ALLOCATOR{
	...
	u16 bucket_bitmap;
	spinlock_t lock;
	struct _EVENT* update_event;
	KERNEL_ATOMIC u16 updated_buckets;
} pmm_allocator_t;
