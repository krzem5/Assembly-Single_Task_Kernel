...
void driver_i82540_init_device(pci_device_t* device){
	...
	i82540_device->rx_desc_base=pmm_alloc(pmm_align_up_address(NUM_RX_DESCRIPTORS*sizeof(i82540_rx_descriptor_t))>>PAGE_SIZE_SHIFT);
	for (u16 i=0;i<NUM_RX_DESCRIPTORS;i++){
		i82540_rx_descriptor_t* desc=GET_DESCRIPTOR(i82540_device,rx,i);
		desc->address=pmm_alloc(1);
		desc->status=0;
	}
	...
	i82540_device->tx_desc_base=pmm_alloc(pmm_align_up_address(NUM_TX_DESCRIPTORS*sizeof(i82540_tx_descriptor_t))>>PAGE_SIZE_SHIFT);
