...
glsl_error_t _glsl_error_create_parser_invalid_constructor(const glsl_ast_node_t* node){
	...
	u32 length=sys_format_string(buffer,ERROR_BUFFER_SIZE,"Type '%s' cannot be constructed from type%s: ",type_str,(node->args.count==1?"":"s"));
	sys_heap_dealloc(NULL,type_str);
	for (u32 i=0;i<node->args.count;i++){
		type_str=glsl_ast_type_to_string(node->args.data[i]->value_type);
...
glsl_error_t _glsl_error_create_parser_too_many_arguments(void){
	char buffer[ERROR_BUFFER_SIZE];
	return _create_error(buffer,sys_format_string(buffer,ERROR_BUFFER_SIZE,"Too many arguments"));
}
