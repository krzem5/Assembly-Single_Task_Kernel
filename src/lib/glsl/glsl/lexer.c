...
SYS_PUBLIC glsl_error_t glsl_lexer_extract_tokens(const char* src,glsl_lexer_token_list_t* out){
	...
	while (src[0]){
		...
		LEXER_CHECK_TOKEN_WORD("bool",GLSL_LEXER_TOKEN_TYPE_BOOL);
		...
		LEXER_CHECK_TOKEN_WORD("float",GLSL_LEXER_TOKEN_TYPE_FLOAT);
		...
		LEXER_CHECK_TOKEN_WORD("int",GLSL_LEXER_TOKEN_TYPE_INT);
		...
		LEXER_CHECK_TOKEN_WORD("void",GLSL_LEXER_TOKEN_TYPE_VOID);
