...
static void _format_int(sys2_var_arg_list_t* va,u8 flags,format_buffer_state_t* out){
	u64 data;
	if (flags&FLAG_SIGN){
		s64 signed_data=((flags&FLAG_LONG)?sys2_var_arg_get(*va,s64):sys2_var_arg_get(*va,s32));
		if (signed_data<0){
		...
		data=((flags&FLAG_LONG)?sys2_var_arg_get(*va,u64):sys2_var_arg_get(*va,u32));
	}
	if (!data){
		_buffer_state_add(out,'0');
		return;
...
SYS2_PUBLIC u32 sys2_format_string(char* buffer,u32 length,const char* template,...){
	sys2_var_arg_list_t va;
	sys2_var_arg_init(va,template);
	u32 out=sys2_format_string_va(buffer,length,template,&va);
...
SYS2_PUBLIC u32 sys2_format_string_va(char* buffer,u32 length,const char* template,sys2_var_arg_list_t* va){
	if (!length){
		return 0;
	}
	...
	while (*template&&out.offset<out.length){
		...
		if (*template=='c'){
			_buffer_state_add(&out,sys2_var_arg_get(*va,int));
		}
		else if (*template=='s'){
			const char* ptr=sys2_var_arg_get(*va,const char*);
			if (!ptr){
				ptr="(null)";
			}
			while (*ptr){
			...
			u8 value=(u8)sys2_var_arg_get(*va,u32);
			...
			u64 address=sys2_var_arg_get(*va,u64);
			u32 shift=64;
			while (shift){
				if (shift==32){
			...
			const u8* guid=sys2_var_arg_get(*va,const u8*);
			for (u8 i=0;i<32;i++){
				if (i==9||i==13||i==17){
					_buffer_state_add(&out,'-');
				}
			...
			const u8* mac_address=sys2_var_arg_get(*va,const u8*);
			for (u8 i=0;i<12;i++){
				if (i&&!(i&1)){
					_buffer_state_add(&out,':');
				}
			...
			u32 ip4=sys2_var_arg_get(*va,u32);
			for (u8 i=0;i<4;i++){
				if (i){
					_buffer_state_add(&out,'.');
				}
