...
void __sys_io_init(void){
	sys_io_input_fd=sys_fd_open(0,"/proc/self/stdin",SYS_FD_FLAG_READ);
	sys_io_output_fd=sys_fd_open(0,"/proc/self/stdout",SYS_FD_FLAG_WRITE);
	sys_io_error_fd=sys_fd_open(0,"/proc/self/stderr",SYS_FD_FLAG_WRITE);
}
...
SYS_PUBLIC s32 sys_io_input(_Bool blocking){
	char buffer[1];
