...
void __sys_io_init(void){
	sys_io_input_fd=sys_fd_open(0,"/dev/ser/in",SYS_FD_FLAG_READ);
	sys_io_output_fd=sys_fd_open(0,"/dev/ser/out",SYS_FD_FLAG_WRITE);
	sys_io_error_fd=sys_fd_open(0,"/dev/ser/out",SYS_FD_FLAG_WRITE);
}
